...
//获取实时信息
function syncStatus(){
    var url = 'http://stadium.dingchangzi.net/index.php/Api/getSyncStatus';
...
//验证订单
function verify(){
     var url = "http://stadium.dingchangzi.net/index.php/Api/verify.html";
...
function getStatus(fid,ftime){
    ...
    return $.getJSON('http://stadium.dingchangzi.net/index.php/Api/getStatus?fid='+fid+'&ftime='+ftime);
}

function setStatus(obj,status,price){
    var color =new Array("dcz-icon icon-common-selled","dcz-icon icon-common-avaliable availiable","dcz-icon selected icon-common-selected","dcz-icon icon-common-vip","dcz-icon icon-common-locked");
    //0不可用 1可预订 2已订 3仅学生

    if(status == '1'){
        if(  obj.getAttribute('class') != 'dcz-icon selected icon-common-selected' ){
            //console.log(obj.getAttribute('class') + color[status]);
            var html = "￥" + price ;
            obj.innerHTML = html;
        ...
        (obj.getAttribute('class') == 'dcz-icon selected icon-common-selected' ) && clearCart();
        obj.innerHTML = '';
...
function iterates(){
    ...
    for (var i = fields.length - 1; i >= 0; i--) {
        ...
        for (var j = 0; j < ftimes.length; j++) {
            ...
            $.getJSON('http://stadium.dingchangzi.net/index.php/Api/getStatus?fid='+fid+'&ftime='+ftime,function(data){
...
function iterates2(date){
    ...
    for (var i = fields.length - 1; i >= 0; i--) {
        ...
        fid = $('.field:eq('+ i +')').attr('fid');
        $.ajaxSettings.async = false;       //设置同步加载
        $.getJSON('http://stadium.dingchangzi.net/index.php/Api/getStatus2?fid='+fid+'&date='+date,function(data){
            for (var j = 0 ; j < ftimes.length  ; j++) {
                obj = ftimes[j];
                ftime = obj.getAttribute('ftime');
                obj.setAttribute('id',fid+ftime);
...
function addToCart(fid,ftime,price,sort){
    ...
    $('.'+fid+ftime).fadeIn();
    $('.book .money-count').val(parseInt($('.book .money-count').val())+price)
...
function select_toggle(e){
    ...
    var is_select = new Array("dcz-icon icon-common-avaliable availiable","dcz-icon selected icon-common-selected");
    var target = e.srcElement ? e.srcElement : e.target;
...
function select_date(date){
    ...
    window.int = window.setInterval("iterates2($('#date').val())",5000);
    $('#date').val(date);
    $(".loading").show();
    $(".sites").html("");
    $.ajaxSettings.async = false;
    html = "";
    $.getJSON('http://stadium.dingchangzi.net/index.php/Api/getDateStatus?date=' + date,function(data){
        $(data).each(function(index){
            var obj = data[index];
            html +='<tr>';
